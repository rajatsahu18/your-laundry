export const services = [
  {
    id: "0",
    image: "https://cdn-icons-png.flaticon.com/128/3003/3003984.png",
    name: "Washing",
  },
  {
    id: "11",
    image: "https://cdn-icons-png.flaticon.com/128/2975/2975175.png",
    name: "Laundry",
  },
  {
    id: "12",
    image: "https://cdn-icons-png.flaticon.com/128/9753/9753675.png",
    name: "Wash & Iron",
  },
  {
    id: "13",
    image: "https://cdn-icons-png.flaticon.com/128/995/995016.png",
    name: "Cleaning",
  },
];

export const carouselImages = [
  "https://images.unsplash.com/photo-1593855648042-850fa7bf86bd?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
  "https://images.unsplash.com/photo-1576109135919-aa9bf1e6a975?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
  "https://plus.unsplash.com/premium_photo-1678218575597-5d0a2810825f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
  "https://media.istockphoto.com/id/187740278/photo/house-cleaning-product.jpg?s=612x612&w=0&k=20&c=5kJnXRmcRevCgQxazgE-3l0IWGd4DfLVm_yhpeo1S0Q=",
  "https://plus.unsplash.com/premium_photo-1682129242439-952d7d2e7902?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
  "https://media.istockphoto.com/id/1247884083/vector/laundry-service-room-vector-illustration-washing-and-drying-machines-with-cleansers-on-shelf.jpg?s=612x612&w=0&k=20&c=myaNEKlqX7R--bzWGDoMI7PhdxG_zdQTKYEBlymJQGk=",
];

export const products = [
  {
    id: "0",
    image: "https://cdn-icons-png.flaticon.com/128/4643/4643574.png",
    name: "shirt",
    quantity: 0,
    price: 10,
  },
  {
    id: "11",
    image: "https://cdn-icons-png.flaticon.com/128/892/892458.png",
    name: "T-shirt",
    quantity: 0,
    price: 10,
  },
  {
    id: "12",
    image: "https://cdn-icons-png.flaticon.com/128/9609/9609161.png",
    name: "dresses",
    quantity: 0,
    price: 10,
  },
  {
    id: "13",
    image: "https://cdn-icons-png.flaticon.com/128/599/599388.png",
    name: "jeans",
    quantity: 0,
    price: 10,
  },
  {
    id: "14",
    image: "https://cdn-icons-png.flaticon.com/128/9431/9431166.png",
    name: "Sweater",
    quantity: 0,
    price: 10,
  },
  {
    id: "15",
    image: "https://cdn-icons-png.flaticon.com/128/3345/3345397.png",
    name: "shorts",
    quantity: 0,
    price: 10,
  },
  {
    id: "16",
    image: "https://cdn-icons-png.flaticon.com/128/293/293241.png",
    name: "Sleeveless",
    quantity: 0,
    price: 10,
  },
];

export const deliveryTime = [
  {
    id: "1",
    name: "2-3 Days",
  },
  {
    id: "2",
    name: "3-4 Days",
  },
  {
    id: "3",
    name: "4-5 Days",
  },
  {
    id: "4",
    name: "5-6 Days",
  },
  {
    id: "5",
    name: "Tommorrow",
  },
];

export const times = [
  {
    id: "1",
    time: "10:00 AM",
  },
  {
    id: "2",
    time: "11:00 AM",
  },
  {
    id: "3",
    time: "12:00 AM",
  },
  {
    id: "4",
    time: "1:00 PM",
  },
  {
    id: "5",
    time: "2:00 PM",
  },
  {
    id: "6",
    time: "3:00 PM",
  },
  {
    id: "7",
    time: "4:00 PM",
  },
];

export const TEXT = {
  ADD: "ADD",
  SERVICE_AVAILABLE: "Services Available",
  YOUR_LAUNDRY: "Your Laundry",
  FOUNDED: "Founded in 2023",
  COMPANY_DESCRIPTION:
    "Welcome to our amazing app! We are a dedicated team of professionals committed to providing you with the best service and experience.",
  OUR_TEAM: "Our Team",
  ADDRESS_BOOK: 'Address Book',
  WELCOME: 'Welcome',
  YOUR_ORDERS: 'Your Orders',
  ABOUT: 'About',
  CONTACT: 'Contact',
  ADDRESS: 'Address',
  LOGOUT: 'Log Out',
  EMPTY_CART: 'Your cart is empty',
  YOUR_BUCKET: 'Your Bucket',
  BILLING_ADDRESS: 'Billing Details',
  ITEM_TOTAL: 'Item Total',
  DELIVERY_FEE: 'Delivery Fee',
  FREE: 'FREE',
  FREE_DELIVERY_TEXT: 'Free Delivery on Your order',
  DELIVERY_CHARGES: 'Delivery charges',
  SELECTED_DATE: 'Pick Up Date',
  SELECTED_TIME: 'Pick Up Time',
  NO_OF_DAYS: 'No of Days',
  TO_PAY:'To Pay',
  PLACE_ORDER: 'Place Order',
  HOME: 'Home',
  PROCEED_TO_PICK: 'Proceed to pickup',
  LOGIN: 'Log In',
  DONT_HAVE_ACCOUNT: "Don't have a account? Sign Up",
  BACK_HOME: 'Back to Home',
  ORDER_PLACED: 'Order Placed!',
  ENTER_ADDRESS: 'Delivery Address',
  DELIVERY_DATE: 'Delivery Date',
  PROCEED_TO_CART: 'Proceed to Cart',
  REGISTER: 'Register',
  ALREADY_HAVE_ACCOUNT: 'Already have a account? Sign in',
  FETCHING_LOCATION: 'Fetching location...'
};

export const about = [
  {
    id: 0,
    name: "Hermione Granger",
    position: "CEO",
    icon: "pencil-square-o",
    image:
      "https://images.unsplash.com/photo-1593104547489-5cfb3839a3b5?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1453&q=80",
  },
  {
    id: 1,
    name: "Rajat Sahu",
    position: "CTO",
    icon: "laptop",
    image:
      "https://rajatsahuportfolio.netlify.app/static/media/cropped.eeaa8196.jpg",
  },
  {
    id: 2,
    name: "Ginny Weasley",
    position: "CMO",
    icon: "laptop",
    image:
      "https://images.unsplash.com/photo-1525875975471-999f65706a10?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80",
  },
  {
    id: 3,
    name: "Neville Longbottom",
    position: "Financial Manager",
    icon: "money",
    image:
      "https://images.unsplash.com/photo-1557862921-37829c790f19?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1471&q=80",
  },
];

export const contact = [
  {
    id: 0,
    url: "`tel:${phoneNumber}`",
    subHeading: "Call customer care between 10:00 AM to 07:00 PM",
    color: "#4EA94B",
    icon: "phone",
    modeOfContact: "7607205292",
  },
  {
    id: 1,
    url: "`mailto:${email}`",
    subHeading:
      "If you any query related to your order kindly drop a mail here.",
    color: "#D14836",
    icon: "envelope",
    modeOfContact: "sahurajat78@gmail.com",
  },
  {
    id: 2,
    url: "github",
    subHeading: "To see more such products kindly visit my GitHub profile",
    color: "#000",
    icon: "github",
    modeOfContact: "https://github.com/rajatsahu18",
  },
  {
    id: 3,
    url: "linkedin",
    subHeading: "To know more about me kindly visit my LinkedIn profile",
    color: "#0077B5",
    icon: "linkedin",
    modeOfContact: "https://www.linkedin.com/in/rajat-sahu-102791147/",
  },
];
